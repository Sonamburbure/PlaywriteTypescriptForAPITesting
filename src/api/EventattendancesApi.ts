...
export class EventAttendanceApi {
    private apiContext:APIRequestContext;
constructor(apiContext:APIRequestContext){
    this.apiContext=apiContext;
}
async createEventAttendance(payload:any){
...
const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/eventattendance`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ eventattendance API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
