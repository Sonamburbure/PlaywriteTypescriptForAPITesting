...
export class StaffCostingApi {
  private apiContext: APIRequestContext;
  private staffcostingid: number | null = null;
  ...
  async createStaffCosting(payload: any) {
    ...
    this.staffcostingid = data?.staffcostingid || data?.staffcosting_id || data?.id;
    console.log('🆔 Stored staffcostingid:', this.staffcostingid);

    return data;
  }

  async getStaffCosting() {
    if (!this.staffcostingid) {
      throw new Error('❌ staffcostingid not found. Call createStaffCosting first.');
    }

    return this.getStaffCostingById(this.staffcostingid);
  }

  async getStaffCostingById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/staffcostings/${id}`,
      { headers }
    );
  ...
  async updateStaffCosting(payload: any) {
    if (!this.staffcostingid) {
      throw new Error('❌ staffcostingid not found. Call createStaffCosting first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/staffcostings/${this.staffcostingid}`,
      { headers, data: payload }
    );
  ...
  async deleteStaffCostingById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchStaffCostings(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
