...
export class stafftypecostingApi {
  ...
  async createstafftypecosting(payload: any) {
    ...
    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/staffcosting`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ stafftypecosting API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
