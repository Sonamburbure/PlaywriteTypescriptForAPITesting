...
export class EventBarCardMenuApi {
    private apiContext:APIRequestContext;
constructor(apiContext:APIRequestContext){
    this.apiContext=apiContext;
}
async createEventBarCardMenu(payload:any){
...
const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/eventbarcardmenu`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ eventbarcardmenu API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
