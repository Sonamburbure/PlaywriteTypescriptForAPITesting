...
export class ItemservedApi {
  ...
  private itemservedid: number | null = null;
  ...
  async createItemserved(payload: any) {
    ...
    this.itemservedid = data?.itemservedid || data?.itemserved_id || data?.id;
    console.log('🆔 Stored itemservedid:', this.itemservedid);

    return data;
  }

  async getItemserved() {
    if (!this.itemservedid) {
      throw new Error('❌ itemservedid not found. Call createItemserved first.');
    }

    return this.getItemservedById(this.itemservedid);
  }

  async getItemservedById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/itemserveds/${id}`,
      { headers }
    );
  ...
  async updateItemserved(payload: any) {
    if (!this.itemservedid) {
      throw new Error('❌ itemservedid not found. Call createItemserved first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/itemserveds/${this.itemservedid}`,
      { headers, data: payload }
    );
  ...
  async deleteItemservedById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchItemserveds(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
