...
export class PackageItemBarApi {
  private apiContext: APIRequestContext;
  private packageitembarid: number | null = null;
  ...
  async createPackageItemBar(payload: any) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/packageitembar`,
    ...
    this.packageitembarid = data?.packageitembarid || data?.packageitembar_id || data?.id;
    console.log('🆔 Stored packageitembarid:', this.packageitembarid);

    return data;
  }

  async getPackageItemBar() {
    if (!this.packageitembarid) {
      throw new Error('❌ packageitembarid not found. Call createPackageItemBar first.');
    }

    return this.getPackageItemBarById(this.packageitembarid);
  }

  async getPackageItemBarById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/packageitembars/${id}`,
      { headers }
    );
  ...
  async updatePackageItemBar(payload: any) {
    if (!this.packageitembarid) {
      throw new Error('❌ packageitembarid not found. Call createPackageItemBar first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/packageitembars/${this.packageitembarid}`,
      { headers, data: payload }
    );
  ...
  async deletePackageItemBarById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchPackageItemBars(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
