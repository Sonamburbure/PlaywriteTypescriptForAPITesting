...
export class SupplierApi {
  ...
  async createSupplier(payload: any) {
    ...
    console.log('🆔 Stored supplierId:', this.supplierId);
  ...
  async getSupplier() {
    if (!this.supplierId) {
      throw new Error('❌ supplierId not found. Call createSupplier first.');
    }

    return this.getSupplierById(this.supplierId);
  }
  ...
  async getSupplierById(id: number) {
    ...
    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/suppliers/${id}`,
      { headers }
    );
  ...
  async updateSupplier(payload: any) {
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/suppliers/${this.supplierId}`,
      { headers, data: payload }
    );
  ...
  async searchSuppliers(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
