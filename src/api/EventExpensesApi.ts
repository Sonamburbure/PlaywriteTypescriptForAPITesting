...
export class EventExpenseAPI {
  ...
  private eventforecastedcostid: number | null = null;
  ...
  async createEventExpense(payload: any) {
    ...
    this.eventforecastedcostid = data?.eventforecastedcostid || data?.eventforecastedcost_id || data?.id;
    console.log('🆔 Stored eventforecastedcostid:', this.eventforecastedcostid);

    return data;
  }

  async getEventExpense() {
    if (!this.eventforecastedcostid) {
      throw new Error('❌ eventforecastedcostid not found. Call createEventExpense first.');
    }

    return this.getEventExpenseById(this.eventforecastedcostid);
  }

  async getEventExpenseById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/eventforecastedcosts/${id}`,
      { headers }
    );
  ...
  async updateEventExpense(payload: any) {
    if (!this.eventforecastedcostid) {
      throw new Error('❌ eventforecastedcostid not found. Call createEventExpense first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/eventforecastedcosts/${this.eventforecastedcostid}`,
      { headers, data: payload }
    );
  ...
  async deleteEventExpenseById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchEventExpenses(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
