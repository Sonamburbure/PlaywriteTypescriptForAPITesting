...
export class OpportunityApi {
  ...
  async createOpportunity(payload: any) {
    ...
    this.opportunityId = data?.opportunityid || data?.opportunity_id || data?.id;
    console.log('🆔 Stored opportunityId:', this.opportunityId);
  ...
  async getOpportunity() {
    if (!this.opportunityId) {
      throw new Error('❌ opportunityId not found. Call createOpportunity first.');
    }

    return this.getOpportunityById(this.opportunityId);
  }
  ...
  async getOpportunityById(id: number) {
    ...
    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/opportunities/${id}`,
      { headers }
    );
  ...
  async updateOpportunity(payload: any) {
    if (!this.opportunityId) {
      throw new Error('❌ opportunityId not found. Call createOpportunity first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/opportunities/${this.opportunityId}`,
      { headers, data: payload }
    );
  ...
  async deleteOpportunityById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchOpportunities(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
