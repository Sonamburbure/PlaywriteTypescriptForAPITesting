...
export class PackagesApi {
  ...
  async createPackage(payload: any) {
    ...
    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/package`,
    ...
    this.packageid = data?.packageid || data?.package_id || data?.id;
    console.log('🆔 Stored packageid:', this.packageid);
  ...
  async getPackage() {
    if (!this.packageid) {
      throw new Error('❌ packageid not found. Call createPackage first.');
    }

    return this.getPackageById(this.packageid);
  }
  ...
  async getPackageById(id: number) {
    ...
    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/packages/${id}`,
      { headers }
    );
  ...
  async updatePackage(payload: any) {
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/packages/${this.packageid}`,
      { headers, data: payload }
    );
  ...
  async searchPackages(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
