...
export class equipmentvendorApi {
  ...
  async createequipmentvendor(payload: any) {
    ...
    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/equipmentvendor`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ equipmentvendor API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
