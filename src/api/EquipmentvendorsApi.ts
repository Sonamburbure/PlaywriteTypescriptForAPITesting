...
export class EquipmentVendorApi {
  ...
  private equipmentvendorid: number | null = null;
  ...
  async createEquipmentVendor(payload: any) {
    ...
    this.equipmentvendorid = data?.equipmentvendorid || data?.equipmentvendor_id || data?.id;
    console.log('🆔 Stored equipmentvendorid:', this.equipmentvendorid);

    return data;
  }

  async getEquipmentVendor() {
    if (!this.equipmentvendorid) {
      throw new Error('❌ equipmentvendorid not found. Call createEquipmentVendor first.');
    }

    return this.getEquipmentVendorById(this.equipmentvendorid);
  }

  async getEquipmentVendorById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/equipmentvendors/${id}`,
      { headers }
    );
  ...
  async updateEquipmentVendor(payload: any) {
    if (!this.equipmentvendorid) {
      throw new Error('❌ equipmentvendorid not found. Call createEquipmentVendor first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/equipmentvendors/${this.equipmentvendorid}`,
      { headers, data: payload }
    );
  ...
  async deleteEquipmentVendorById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchEquipmentVendors(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
