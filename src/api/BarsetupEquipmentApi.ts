...
export class BarsetupEquipmentApi {
  ...
  private barsetupequipmentid: number | null = null;
  ...
  async createBarsetupEquipment(payload: any) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/barsetupequipment`,
    ...
    this.barsetupequipmentid = data?.barsetupequipmentid || data?.barsetupequipment_id || data?.id;
    console.log('🆔 Stored barsetupequipmentid:', this.barsetupequipmentid);

    return data;
  }

  async getBarsetupEquipment() {
    if (!this.barsetupequipmentid) {
      throw new Error('❌ barsetupequipmentid not found. Call createBarsetupEquipment first.');
    }

    return this.getBarsetupEquipmentById(this.barsetupequipmentid);
  }

  async getBarsetupEquipmentById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/barsetupequipments/${id}`,
      { headers }
    );
  ...
  async updateBarsetupEquipment(payload: any) {
    if (!this.barsetupequipmentid) {
      throw new Error('❌ barsetupequipmentid not found. Call createBarsetupEquipment first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/barsetupequipments/${this.barsetupequipmentid}`,
      { headers, data: payload }
    );
  ...
  async deleteBarsetupEquipmentById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchBarsetupEquipments(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
