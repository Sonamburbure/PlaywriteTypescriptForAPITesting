...
export class EventChecklistApi {
  ...
  private eventchecklistid: number | null = null;
  ...
  async createEventChecklist(payload: any) {
    ...
    this.eventchecklistid = data?.eventchecklistid || data?.eventchecklist_id || data?.id;
    console.log('🆔 Stored eventchecklistid:', this.eventchecklistid);

    return data;
  }

  async getEventChecklist() {
    if (!this.eventchecklistid) {
      throw new Error('❌ eventchecklistid not found. Call createEventChecklist first.');
    }

    return this.getEventChecklistById(this.eventchecklistid);
  }

  async getEventChecklistById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/eventchecklists/${id}`,
      { headers }
    );
  ...
  async updateEventChecklist(payload: any) {
    if (!this.eventchecklistid) {
      throw new Error('❌ eventchecklistid not found. Call createEventChecklist first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/eventchecklists/${this.eventchecklistid}`,
      { headers, data: payload }
    );
  ...
  async deleteEventChecklistById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchEventChecklists(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
