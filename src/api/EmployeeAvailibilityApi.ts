...
export class EmployeeAvailabilityApi {
  ...
  private employeeavailabilityId: number | null = null;
  ...
  async createEmployeeAvailability(payload: any) {
    ...
    this.employeeavailabilityId = data?.employeeavailabilityid || data?.employeeavailability_id || data?.id;
    console.log('🆔 Stored employeeavailabilityId:', this.employeeavailabilityId);

    return data;
  }

  async getEmployeeAvailability() {
    if (!this.employeeavailabilityId) {
      throw new Error('❌ employeeavailabilityId not found. Call createEmployeeAvailability first.');
    }

    return this.getEmployeeAvailabilityById(this.employeeavailabilityId);
  }

  async getEmployeeAvailabilityById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/employeeavailabilities/${id}`,
      { headers }
    );
  ...
  async updateEmployeeAvailability(payload: any) {
    if (!this.employeeavailabilityId) {
      throw new Error('❌ employeeavailabilityId not found. Call createEmployeeAvailability first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/employeeavailabilities/${this.employeeavailabilityId}`,
      { headers, data: payload }
    );
  ...
  async deleteEmployeeAvailabilityById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
