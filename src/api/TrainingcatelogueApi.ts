...
export class TrainingCatalogueApi {
  ...
  private trainingid: number | null = null;
  ...
  async createTrainingCatalogue(payload: any) {
    ...
    this.trainingid = data?.trainingid || data?.training_id || data?.id;
    console.log('🆔 Stored trainingid:', this.trainingid);

    return data;
  }

  async getTrainingCatalogue() {
    if (!this.trainingid) {
      throw new Error('❌ trainingid not found. Call createTrainingCatalogue first.');
    }

    return this.getTrainingCatalogueById(this.trainingid);
  }

  async getTrainingCatalogueById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/trainings/${id}`,
      { headers }
    );
  ...
  async updateTrainingCatalogue(payload: any) {
    if (!this.trainingid) {
      throw new Error('❌ trainingid not found. Call createTrainingCatalogue first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/trainings/${this.trainingid}`,
      { headers, data: payload }
    );
  ...
  async deleteTrainingCatalogueById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchTrainingCatalogues(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
