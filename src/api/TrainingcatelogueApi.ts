...
export class TrainingCatalogueApi {
  ...
  async createTrainingCatalogue(payload: any) {
    ...
    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/training`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ Training Catalogue API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
