...
export class VenueApi {
  ...
  async createVenue(payload: any) {
    ...
    this.venueId = data?.venueid || data?.venue_id || data?.id;
    console.log('🆔 Stored venueId:', this.venueId);
  ...
  async getVenue() {
    if (!this.venueId) {
      throw new Error('❌ venueId not found. Call createVenue first.');
    }

    return this.getVenueById(this.venueId);
  }
  ...
  async getVenueById(id: number) {
    ...
    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/venues/${id}`,
      { headers }
    );
  ...
  async updateVenue(payload: any) {
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/venues/${this.venueId}`,
      { headers, data: payload }
    );
  ...
  async searchVenues(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
