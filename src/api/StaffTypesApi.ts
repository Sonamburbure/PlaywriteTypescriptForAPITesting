...
export class StaffTypeApi {
  private apiContext: APIRequestContext;
  private stafftypeid: number | null = null;
  ...
  async createStaffType(payload: any) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/stafftype`,
    ...
    this.stafftypeid = data?.stafftypeid || data?.stafftype_id || data?.id;
    console.log('🆔 Stored stafftypeid:', this.stafftypeid);
  ...
  async getStaffType() {
    if (!this.stafftypeid) {
      throw new Error('❌ stafftypeid not found. Call createStaffType first.');
    }

    return this.getStaffTypeById(this.stafftypeid);
  }

  async getStaffTypeById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/stafftypes/${id}`,
      { headers }
    );
  ...
  async updateStaffType(payload: any) {
    if (!this.stafftypeid) {
      throw new Error('❌ stafftypeid not found. Call createStaffType first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/stafftypes/${this.stafftypeid}`,
      { headers, data: payload }
    );
  ...
  async deleteStaffTypeById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchStaffTypes(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
