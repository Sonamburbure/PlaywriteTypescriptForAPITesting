...
export class EventBarCardDateApi {
    private apiContext:APIRequestContext;
constructor(apiContext:APIRequestContext){
    this.apiContext=apiContext;
}
async createEventBarCardDate(payload:any){
...
const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/eventbarcarddate`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ eventbarcarddate API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
