...
export class ItemservedStaffingApi {
  ...
  private itemstaffid: number | null = null;
  ...
  async createItemservedStaffing(payload: any) {
    ...
    this.itemstaffid = data?.itemstaffid || data?.itemstaff_id || data?.id;
    console.log('🆔 Stored itemstaffid:', this.itemstaffid);

    return data;
  }

  async getItemservedStaffing() {
    if (!this.itemstaffid) {
      throw new Error('❌ itemstaffid not found. Call createItemservedStaffing first.');
    }

    return this.getItemservedStaffingById(this.itemstaffid);
  }

  async getItemservedStaffingById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/itemstaffs/${id}`,
      { headers }
    );
  ...
  async updateItemservedStaffing(payload: any) {
    if (!this.itemstaffid) {
      throw new Error('❌ itemstaffid not found. Call createItemservedStaffing first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/itemstaffs/${this.itemstaffid}`,
      { headers, data: payload }
    );
  ...
  async deleteItemservedStaffingById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchItemservedStaffings(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
