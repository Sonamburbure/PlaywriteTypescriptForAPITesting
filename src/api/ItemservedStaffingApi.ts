...
export class itemservedstaffingApi {
  ...
  async createItemservedstaffing(payload: any) {
    ...
    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/itemstaff`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ itemservedstaffing API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
