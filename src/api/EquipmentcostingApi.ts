...
export class EquipmentCostingApi {
  private apiContext: APIRequestContext;
  private equipmentcostingid: number | null = null;
  ...
  async createEquipmentCosting(payload: any) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/equipmentcosting`,
    ...
    this.equipmentcostingid = data?.equipmentcostingid || data?.equipmentcosting_id || data?.id;
    console.log('🆔 Stored equipmentcostingid:', this.equipmentcostingid);

    return data;
  }

  async getEquipmentCosting() {
    if (!this.equipmentcostingid) {
      throw new Error('❌ equipmentcostingid not found. Call createEquipmentCosting first.');
    }

    return this.getEquipmentCostingById(this.equipmentcostingid);
  }

  async getEquipmentCostingById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/equipmentcostings/${id}`,
      { headers }
    );
  ...
  async updateEquipmentCosting(payload: any) {
    if (!this.equipmentcostingid) {
      throw new Error('❌ equipmentcostingid not found. Call createEquipmentCosting first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/equipmentcostings/${this.equipmentcostingid}`,
      { headers, data: payload }
    );
  ...
  async deleteEquipmentCostingById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchEquipmentCostings(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
