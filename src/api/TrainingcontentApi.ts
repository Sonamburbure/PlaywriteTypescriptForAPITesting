...
export class TrainingContentApi {
  ...
  private trainingtopicid: number | null = null;
  ...
  async createTrainingContent(payload: any) {
    ...
    this.trainingtopicid = data?.trainingtopicid || data?.trainingtopic_id || data?.id;
    console.log('🆔 Stored trainingtopicid:', this.trainingtopicid);
  ...
  async getTrainingContent() {
    if (!this.trainingtopicid) {
      throw new Error('❌ trainingtopicid not found. Call createTrainingContent first.');
    }

    return this.getTrainingContentById(this.trainingtopicid);
  }

  async getTrainingContentById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/trainingtopics/${id}`,
      { headers }
    );
  ...
  async updateTrainingContent(payload: any) {
    if (!this.trainingtopicid) {
      throw new Error('❌ trainingtopicid not found. Call createTrainingContent first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/trainingtopics/${this.trainingtopicid}`,
      { headers, data: payload }
    );
  ...
  async deleteTrainingContentById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchTrainingContents(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
