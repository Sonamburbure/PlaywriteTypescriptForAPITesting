...
export class trainingcontentApi {
  ...
  async createTrainingContent(payload: any) {
    ...
    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/trainingtopic`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ Training Content API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
