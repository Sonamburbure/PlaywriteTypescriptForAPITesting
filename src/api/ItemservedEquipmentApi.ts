...
export class itemservedEquipmentApi {
  ...
  async createItemservedEquipment(payload: any) {
    ...
    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/itemproduct`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ itemservedEquipment API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
