...
export class ItemservedproductApi {
  ...
  private itemproductid: number | null = null;
  ...
  async createItemservedproduct(payload: any) {
    ...
    this.itemproductid = data?.itemproductid || data?.itemproduct_id || data?.id;
    console.log('🆔 Stored itemproductid:', this.itemproductid);

    return data;
  }

  async getItemservedproduct() {
    if (!this.itemproductid) {
      throw new Error('❌ itemproductid not found. Call createItemservedproduct first.');
    }

    return this.getItemservedproductById(this.itemproductid);
  }

  async getItemservedproductById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/itemproducts/${id}`,
      { headers }
    );
  ...
  async updateItemservedproduct(payload: any) {
    if (!this.itemproductid) {
      throw new Error('❌ itemproductid not found. Call createItemservedproduct first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/itemproducts/${this.itemproductid}`,
      { headers, data: payload }
    );
  ...
  async deleteItemservedproductById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchItemservedproducts(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
