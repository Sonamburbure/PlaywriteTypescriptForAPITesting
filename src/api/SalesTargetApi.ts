...
export class SalesTargetApi {
  private apiContext: APIRequestContext;
  private salestargetId: number | null = null;
  ...
  async createSalesTarget(payload: any) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/salestarget`,
      { headers, data: payload }
    );

    const data = await this.handleResponse(response, 'CREATE');

    this.salestargetId = data?.salestargetid || data?.salestarget_id || data?.id;
    console.log('🆔 Stored salestargetId:', this.salestargetId);
  ...
  async getSalesTarget() {
    if (!this.salestargetId) {
      throw new Error('❌ salestargetId not found. Call createSalesTarget first.');
    }

    return this.getSalesTargetById(this.salestargetId);
  }

  async getSalesTargetById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/salestargets/${id}`,
      { headers }
    );
  ...
  async updateSalesTarget(payload: any) {
    if (!this.salestargetId) {
      throw new Error('❌ salestargetId not found. Call createSalesTarget first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/salestargets/${this.salestargetId}`,
      { headers, data: payload }
    );
  ...
  async deleteSalesTargetById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchSalesTargets(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
