...
export class Segment1TuomApi {
  private apiContext: APIRequestContext;
  private segment1tuomid: number | null = null;
  ...
  async createSegment1Tuom(payload: any) {
    ...
    this.segment1tuomid = data?.segment1tuomid || data?.segment1tuom_id || data?.id;
    console.log('🆔 Stored segment1tuomid:', this.segment1tuomid);

    return data;
  }

  async getSegment1Tuom() {
    if (!this.segment1tuomid) {
      throw new Error('❌ segment1tuomid not found. Call createSegment1Tuom first.');
    }

    return this.getSegment1TuomById(this.segment1tuomid);
  }

  async getSegment1TuomById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/segment1tuoms/${id}`,
      { headers }
    );
  ...
  async updateSegment1Tuom(payload: any) {
    if (!this.segment1tuomid) {
      throw new Error('❌ segment1tuomid not found. Call createSegment1Tuom first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/segment1tuoms/${this.segment1tuomid}`,
      { headers, data: payload }
    );
  ...
  async deleteSegment1TuomById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchSegment1Tuoms(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
