...
export class BarsetupApi {
  ...
  private barsetupid: number | null = null;
  ...
  async createBarsetup(payload: any) {
    ...
    this.barsetupid = data?.barsetupid || data?.barsetup_id || data?.id;
    console.log('🆔 Stored barsetupid:', this.barsetupid);
  ...
  async getBarsetup() {
    if (!this.barsetupid) {
      throw new Error('❌ barsetupid not found. Call createBarsetup first.');
    }

    return this.getBarsetupById(this.barsetupid);
  }

  async getBarsetupById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/barsetups/${id}`,
      { headers }
    );
  ...
  async updateBarsetup(payload: any) {
    if (!this.barsetupid) {
      throw new Error('❌ barsetupid not found. Call createBarsetup first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/barsetups/${this.barsetupid}`,
      { headers, data: payload }
    );
  ...
  async deleteBarsetupById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchBarsetups(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
