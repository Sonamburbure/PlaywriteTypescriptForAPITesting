...
export class ProductCostingApi {
  ...
  private productcostingid: number | null = null;
  ...
  async createProductCosting(payload: any) {
    ...
    this.productcostingid = data?.productcostingid || data?.productcosting_id || data?.id;
    console.log('🆔 Stored productcostingid:', this.productcostingid);

    return data;
  }

  async getProductCosting() {
    if (!this.productcostingid) {
      throw new Error('❌ productcostingid not found. Call createProductCosting first.');
    }

    return this.getProductCostingById(this.productcostingid);
  }

  async getProductCostingById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/productcostings/${id}`,
      { headers }
    );
  ...
  async updateProductCosting(payload: any) {
    if (!this.productcostingid) {
      throw new Error('❌ productcostingid not found. Call createProductCosting first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/productcostings/${this.productcostingid}`,
      { headers, data: payload }
    );
  ...
  async deleteProductCostingById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchProductCostings(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
