...
export class ChecklistMasterApi {
  ...
  private checklistmasterid: number | null = null;
  ...
  async createChecklistMaster(payload: any) {
    ...
    this.checklistmasterid = data?.checklistmasterid || data?.checklistmaster_id || data?.id;
    console.log('🆔 Stored checklistmasterid:', this.checklistmasterid);

    return data;
  }

  async getChecklistMaster() {
    if (!this.checklistmasterid) {
      throw new Error('❌ checklistmasterid not found. Call createChecklistMaster first.');
    }

    return this.getChecklistMasterById(this.checklistmasterid);
  }

  async getChecklistMasterById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/checklistmasters/${id}`,
      { headers }
    );
  ...
  async updateChecklistMaster(payload: any) {
    if (!this.checklistmasterid) {
      throw new Error('❌ checklistmasterid not found. Call createChecklistMaster first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/checklistmasters/${this.checklistmasterid}`,
      { headers, data: payload }
    );
  ...
  async deleteChecklistMasterById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchChecklistMasters(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
