...
export class checklistmasterApi {
  ...
  async createchecklistmaster(payload: any) {
    ...
    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/checklistmaster`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ checklistmaster API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
