...
export class TaskMasterApi {
  ...
  private taskmasterid: number | null = null;
  ...
  async createTaskMaster(payload: any) {
    ...
    this.taskmasterid = data?.taskmasterid || data?.taskmaster_id || data?.id;
    console.log('🆔 Stored taskmasterid:', this.taskmasterid);
  ...
  async getTaskMaster() {
    if (!this.taskmasterid) {
      throw new Error('❌ taskmasterid not found. Call createTaskMaster first.');
    }

    return this.getTaskMasterById(this.taskmasterid);
  }

  async getTaskMasterById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/taskmasters/${id}`,
      { headers }
    );
  ...
  async updateTaskMaster(payload: any) {
    if (!this.taskmasterid) {
      throw new Error('❌ taskmasterid not found. Call createTaskMaster first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/taskmasters/${this.taskmasterid}`,
      { headers, data: payload }
    );
  ...
  async deleteTaskMasterById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchTaskMasters(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
