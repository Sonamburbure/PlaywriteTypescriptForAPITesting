...
export class EventTaskApi {
  ...
  private eventtaskid: number | null = null;
  ...
  async createEventTask(payload: any) {
    ...
    this.eventtaskid = data?.eventtaskid || data?.eventtask_id || data?.id;
    console.log('🆔 Stored eventtaskid:', this.eventtaskid);
  ...
  async getEventTask() {
    if (!this.eventtaskid) {
      throw new Error('❌ eventtaskid not found. Call createEventTask first.');
    }

    return this.getEventTaskById(this.eventtaskid);
  }

  async getEventTaskById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/eventtasks/${id}`,
      { headers }
    );
  ...
  async updateEventTask(payload: any) {
    if (!this.eventtaskid) {
      throw new Error('❌ eventtaskid not found. Call createEventTask first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/eventtasks/${this.eventtaskid}`,
      { headers, data: payload }
    );
  ...
  async deleteEventTaskById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchEventTasks(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
