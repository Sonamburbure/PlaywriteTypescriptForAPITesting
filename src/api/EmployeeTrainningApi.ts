...
export class EmployeeTrainingApi {
  ...
  private employeetrainingid: number | null = null;
  ...
  async createEmployeeTraining(payload: any) {
    ...
    this.employeetrainingid = data?.employeetrainingid || data?.employeetraining_id || data?.id;
    console.log('🆔 Stored employeetrainingid:', this.employeetrainingid);

    return data;
  }

  async getEmployeeTraining() {
    if (!this.employeetrainingid) {
      throw new Error('❌ employeetrainingid not found. Call createEmployeeTraining first.');
    }

    return this.getEmployeeTrainingById(this.employeetrainingid);
  }

  async getEmployeeTrainingById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();

    const response = await this.apiContext.get(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/employeetrainings/${id}`,
      { headers }
    );
  ...
  async updateEmployeeTraining(payload: any) {
    if (!this.employeetrainingid) {
      throw new Error('❌ employeetrainingid not found. Call createEmployeeTraining first.');
    }
    ...
    const response = await this.apiContext.put(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/employeetrainings/${this.employeetrainingid}`,
      { headers, data: payload }
    );
  ...
  async deleteEmployeeTrainingById(id: number) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
  ...
  async searchEmployeeTrainings(filter: string, ipp: number = 25, page: number = 1) {
    const { tenantPath, logonAs, headers } = this.getHeaders();
