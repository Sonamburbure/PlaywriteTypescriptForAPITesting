...
export class EmployeeTrainingApi {
  ...
  async createEmployeeTraining(payload: any) {
    ...
    const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/employeetraining`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ employeetraining API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
