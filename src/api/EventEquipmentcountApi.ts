...
export class EventEquipmentCountApi {
    private apiContext:APIRequestContext;
constructor(apiContext:APIRequestContext){
    this.apiContext=apiContext;
}
async createEventEquipmentCount(payload:any){
...
const response = await this.apiContext.post(
      `${BASE_API_URL}/${tenantPath}/api/${logonAs}/equipmentcount`,
      {
        headers: {
    ...
    if (!response.ok()) {
      throw new Error(
        `❌ eventequipmentcount API failed: ${response.status()} \n${JSON.stringify(responseBody)}`
      );
    }
